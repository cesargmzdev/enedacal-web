...
const link = (link, string) => {
  return <a className="hover:text-yellow-500 hover:underline duration-300" href={`${link}`} target="_blank" rel="noreferrer">{string}</a>;
};
...
const getProjectProps = (project) => {
  ...
  switch (project) {
    case "bln":
      return {
        title: "BERLIN",
        description: "Seeking for a new visual identity for the city where I live, Berlin, I made the following proposal during my Master Studium starting from the strategy: simplicity, power and versatility. Translating this concepts in to the graphic field, I worked on the 3 most important elements: typography, colours and the icon (bear).",
        src: [
          {type: 'video', src: BLN},
    ...
    case "mdm":
      return {
        title: "MADM",
        description: `Rebrand proposal for Museo de Artes Decoraticas de Madrid. Collab with `,
        src: [
    ...
    case "rukatype":
      return {
        title: "RUKATYPE",
        description: "Rukatype is a typography that highlights the culture and symbology of the Mapuche people. This typography generates some mosaics and patterns that offer a journey to the origin of the mapuche visual culture.Check * the project",
        src: [
        ...
        <section className="p-5 pt-16 font-MontrealLight">
          ...
          <div className="lg:inline-flex gap-x-7">
            ...
            <p className="mt-8 lg:mt-0 text-balance pb-5">
              Lorem ipsum dolor sit amet consectetur adipisicing elit. Dolorum architecto tenetur cupiditate distinctio sequi ab, veritatis excepturi enim atque esse quis vel consequatur voluptate aperiam sint. Possimus laborum ducimus non.
            </p>
          </div>
        </section>
        <section>
          {projectProps.src.map((src, index) => {
            ...
            const lastItemClass = isLastItem ? '' : 'pb-16';
